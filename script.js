async function SearchWeather(city) {
  const apiId = "271d473915a1802dab545af2c4703b5c";
  const units = "metric";

  const response = await fetch(
    `http://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${apiId}&units=${units}`
  );
...
function ShowWeatherInfo(info) {
  ...
  document.getElementById(
    "country-flag"
  ).src = `https://www.countryflags.io/${info.sys.country}/flat/64.png`;

  switch (info.weather[0].main) {
    case "Clear":
      document.getElementById("weather-animation").src =
        "./animations/night.svg";
      document.body.style.backgroundImage = "url(./images/clear.jpg)";
      document.getElementById("weather-description").innerText = "Tempo Aberto";
      break;
    case "Clouds":
      document.getElementById("weather-animation").src =
        "./animations/cloudy.svg";
...
async function handleSearch(city) {
  ...
  } else alert("Digite o nome da cidade!");
}

document
  .getElementById("button-search")
  .addEventListener("click", () =>
    handleSearch(document.getElementById("input-search").value)
  );
